...
ChatFrame.Position               = UDim2.new(1, -420, 0, 54)
...
local function createBubble(senderName, message)
	-- Não cria bubble se o chat acabou de ser aberto (mensagens antigas do poll)
...
task.spawn(function()
	...
	-- ── Primeira sincronização: descubra o último ID sem exibir nada ──
	-- Isso garante que ao abrir, mensagens antigas não gerem bubbles
	local resInit = httpGet("/messages?after=0")
	if resInit and resInit.StatusCode == 200 then
		local ok, data = pcall(function()
			return HttpService:JSONDecode(resInit.Body)
		end)
		if ok and data and data.messages and #data.messages > 0 then
			-- Exibe no painel (histórico), mas NÃO cria bubbles (chatJustOpened = true)
			chatJustOpened = true
			...
			-- Libera flag após processar histórico inicial
...
-- ============================================================
--  MENSAGEM DE BOAS-VINDAS (só no painel, sem bubble)
-- ============================================================
createMessageDisplay("Sistema", "Bem-vindo! Pressione / para digitar. Conectando ao servidor...", true)
